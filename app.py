...
# INTERFACE INTERACTIVE v8.2
st.title(" 🏇 PMU Pro Suite Ultimate Hub v8.2")
st.info(f"👥 **Nombre total de visiteurs sur la plateforme : {st.session_state.total_visiteurs}**")

# Encart publicitaire haut de page coloré
st.markdown("""
...
with onglet3:
    ...
    if not st.session_state.video_vue and not st.session_state.est_admin:
        ...
        for num in list(liste_9_chiffres):
            ...
            # Traitement sécurisé sans structures imbriquées complexes
            if 1 <= num <= 4:
                note_regle = "Favori Solide."
                confiance += 20
            if num == 5:
                note_regle = "Point d'appui incontournable."
                confiance += 15
