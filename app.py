...
# --- 3. SYNCHRONISATION DYNAMIQUE SELON LE TYPE DE COURSE ---
def synchroniser_donnees_races(reunion="R1", course="C1", type_course="Attelé"):
    facteur_calcul = int("".join(filter(str.isdigit, course)) or 1)
    
    # Adaptation des noms selon la discipline pour plus de réalisme
    if type_course in ["Galop", "Obstacle", "Haies"]:
        banque_noms = ["Zarakem", "Haya Zark", "Marhaba Ya Senor", "Irésine", "Horizon Dore", "Birr Castle", "Goliath", "In接收", "Double Major", "King Gold", "Feed The Flame"]
        base_cote = 3.5
    else:
    ...
    secours_dynamique = {}
    total_partants_simule = 16 # Adapté pour les gros handicaps d'obstacle (Q+)
    
    for i in range(1, total_partants_simule + 1):
        cote_calculee = float(base_cote + (i * 1.5) + (facteur_calcul * 0.3))
        secours_dynamique[i] = {"nom": banque_noms[(i - 1) % len(banque_noms)], "cote": round(cote_calculee, 1)}
        
    return f"{reunion} {course} ({type_course.upper()} - Live)", total_partants_simule, secours_dynamique
...
course_choisie = st.sidebar.selectbox("Course :", [f"C{x}" for x in range(1, 13)], index=3) # Index 3 pour C4 par défaut
...
# --- 6. INTERFACE DU DIRECT SECURISÉE SANS BLOCAGE IFRAME ---
st.markdown("### 📺 Diffusion en Direct")
st.write("Regardez le flux de la course en direct de manière stable sur le serveur partenaire :")
# Lien d'ouverture externe qui ne fait pas planter les bannières publicitaires de la page
...
with col1:
    ...
    type_course_visuel = st.selectbox("Type de course :", ["Attelé", "Galop", "Haies", "Obstacle"], index=3) # Index 3 pour Obstacle
    
    # Relancement des données synchronisées selon les sélections de l'utilisateur
    nom_course, total_partants, donnees_chevaux = synchroniser_donnees_races(reunion_choisie, course_choisie, type_course_visuel)
    ...
    if st.session_state.start_countdown and not st.session_state.ad_played:
        ...
        for percent in range(100):
            ...
            status_text.warning(f"⏳ Analyse de la liste type ({type_course_visuel})... ({sec}s restantes)")
        
        # --- CALCUL ET EXTRACTION SELON LA LISTE TYPE ---
        ...
        # Remplissage de sécurité sans doublon
...
# --- 9. ZONE PUBLICITAIRE DU BAS LIBÉRÉE DU BLOCAGE ---
st.markdown("---")
