...
with col2:
    ...
    if not st.session_state.ad_played:
        ...
        # --- CORRECTION DE L'EXTRACTION DES INDEX DE LA LISTE TYPE ---
        cotes_triees = sorted(donnees_chevaux.items(), key=lambda x: x[1]["cote"])
        num_liste_type = [int(item[0]) for item in cotes_triees]  # EXTRACTION DU NUMÉRO CORRECTEMENT

        combinaison_ia_9 = []

        # Application de tes paliers liste type sur les identifiants
