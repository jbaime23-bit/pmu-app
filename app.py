...
def generer_9_chiffres_depuis_remarques(mises_dict, specialite):
    ...
    for k in mises_dict.keys():
        try:
            # Extraction propre des numéros via les expressions régulières
            trouve = re.findall(r'\d+', k)
            if trouve:
                numeros_disponibles.append(int(trouve[0]))
        except:
            continue
...
# INITIALISATION DES COURSES AUTOMATIQUES DU JOUR
