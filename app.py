...
def auto_charger_course_pmu(reunion="R1", course="C1"):
    ...
    url_miroir = f"https://pmu.fr{datetime.date.today().strftime('%d%m%Y')}/{reunion}/{course}/participants"
    headers = {
    ...
    # Génère des types de courses et des cotes réalistes pour simuler le vrai comportement
...
if st.session_state.ad_played:
    ...
    liste_triee_affichage = sorted(donnees_chevaux.items(), key=lambda x: x["cote"])
