...
# --- NOUVEL ALGORITHME DE TRI DYNAMIQUE PAR COTES ET SÉCURISATION DE L'IA ---
num_ia_trie = []
# Tri basé strictement sur la valeur de la cote numérique réelle calculée
cotes_triees_par_valeur = sorted(cotes_s.items(), key=lambda x: x[1])

for cheval_nom, _ in cotes_triees_par_valeur:
    # Extraction sécurisée des caractères numériques de la clé globale
    chiffres_seuls = "".join([c for c in cheval_nom if c.isdigit()])
    if chiffres_seuls.isdigit():
        num_ia_trie.append(int(chiffres_seuls))
...
# --- LISTE DES PARTICIPANTS CORRIGÉE (100% SÉCURISÉE ET LINÉAIRE SANS REPLACEMENTS TEXTUELS) ---
...
for cheval_complet, mise_calc in st.session_state.mises.items():
    # Nettoyage linéaire et sécurisé
    chiffres_num = "".join([char for char in cheval_complet.split(" - ")[0] if char.isdigit()])
    nom_propre_cheval = cheval_complet.split(" - ")[1] if " - " in cheval_complet else cheval_complet
    cote_associee = cotes_s.get(cheval_complet, 99.0)
