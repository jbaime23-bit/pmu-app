...
# BARRE LATÉRALE - ESPACE STATUT SANS PUBLICITÉ BRISÉE
st.sidebar.markdown("---")
...
# Tri et traitement sécurisé de la combinaison IA sur le tuple cotes_s.items()
cotes_triees = sorted(cotes_s.items(), key=lambda item: item[1])
num_ia_trie = []
for key, value in cotes_triees:
    num_extraction = "".join(filter(str.isdigit, key.split(" - ")[0]))
...
# --- BANNIÈRE PUBLICITAIRE N°1 RESTAURÉE (728x90 ORIGINAL DU PMU SANS IFRAME BRISÉE) ---
...
# --- STRUCTURE DU LECTEUR DE COMPTE À REBOURS DU COMPTE DE SECOURS ---
...
if st.session_state.start_countdown and not st.session_state.ad_played:
    ...
    # Exécution du script Popunder Adsterra pendant le décompte
    code_popunder_video = """
    <script type="text/javascript" src="https://profitableratecpmnetwork.com"></script>
    """
...
with onglet1:
    ...
    # RESTAURATION DU CODE EXACT DE LA BANNIÈRE NATIVE DE VOTRE SITE (SANS L'IFRAME QUI CRÉAIT LE CARRÉ GRIS)
    code_adsterra_native_original = """
    ...
    """
    components.html(code_adsterra_native_original, height=180, scrolling=False)
