...
# Personnalisation de l'interface (Design Sombre & Pleine Largeur)
st.markdown("""
<style>
...
st.sidebar.markdown("---")
# CORRECTION SIDEBAR : Remplacement du grand bloc 728px par une annonce textuelle adaptative pour mobile
st.sidebar.markdown("""
<div style="text-align:center; background-color:#1e293b; padding:15px; border-radius:8px; border:1px solid #3b82f6;">
  <b style="color:#3b82f6; font-size:14px;">📊 MONÉTISATION ACTIVE</b><br>
  <small style="color:#94a3b8;">Fenêtre publicitaire principale en cours de diffusion au centre de l'écran.</small>
</div>
""", unsafe_allow_html=True)
st.sidebar.markdown("---")
...
# --- LE HUB CENTRAL ---
st.markdown(f'<div class="main-title">🏇 PMU Pro Suite Ultimate Hub v15.0</div>', unsafe_allow_html=True)

# --- [PUBLICITÉ 1] : BANNIÈRE HAUTE ENCAPSULÉE SÉCURISÉE SANS CONFLIT DE LARGEUR ---
...
"""
components.html(code_adsterra_banner_728, height=110, scrolling=True)
...
# --- [PUBLICITÉ 3] : POPUNDER INTERSTITIEL ---
code_popunder_video = """
...
# --- ALGORITHME DE SÉLECTION IA ---
liste_triee = sorted(donnees_chevaux.items(), key=lambda x: x["cote"])
num_liste_type = [int(item) for item in liste_triee]
