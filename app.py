...
def extraire_un_numero(texte):
    ...
    if chiffres:
        return int(chiffres[0]) # CORRECTION ESSENTIELLE : Extrait le premier élément de la liste
    return 1
...
def generer_9_chiffres_depuis_remarques(mises_dict, specialite):
    ...
    if zone_1:
        try:
            nb_a_prendre = random.choice([1, 2]) # CORRECTION : Ajout des valeurs dans random.choice
    ...
    if zone_3:
        try:
            nb_a_prendre_z3 = random.choice([1, 2]) # CORRECTION : Ajout des valeurs dans random.choice
...
# CHARGEMENT AUTOMATIQUE DES COURSES AU DEMARRAGE
...
# --- VISUEL EN LIGNE COMPACT ---
st.title(f"🏇 PMU Pro Hub v8.4  ·  🟢 {st.session_state.total_visiteurs} en ligne")

# --- LE SCRIPT ADSTERRA EMBARQUÉ ET SÉCURISÉ ---
...
# --- CONFIGURATION DU MINUTEUR DE PUBLICITÉ DE 5 SECONDES ---
