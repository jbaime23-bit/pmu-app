...
# --- BANNIÈRE PUBLICITAIRE N°1 : 728x90 ---
...
# --- DISPOSITIF COMPTE À REBOURS ET LIEN DIRECT PUBLICITAIRE ---
...
# REMPLACEZ l'adresse ci-dessous par votre propre "Lien Direct / Direct Link" Adsterra si vous en générez un nouveau
lien_direct_adsterra = "https://profitableratecpmnetwork.com"
...
if not st.session_state.start_countdown and not st.session_state.ad_played:
    # Ce bouton HTML ouvre proprement la publicité Adsterra dans un nouvel onglet et déclenche le compte à rebours dans l'application
    html_bouton_ad = f"""
    <a href="{lien_direct_adsterra}" target="_blank" style="text-decoration:none;">
        <button style="background-color:#ef4444; color:white; border:none; padding:14px 25px; font-size:16px; font-weight:bold; border-radius:6px; cursor:pointer; width:100%;">
            ▶️ LANCER LE STREAMING EN DIRECT EQUIDIA (SPONSORISÉ)
        </button>
    </a>
    """
    st.markdown(html_bouton_ad, unsafe_allow_html=True)
    
    # Petit bouton Streamlit pour démarrer les 5 secondes juste après que l'utilisateur a cliqué
    if st.button("⚡ Valider et Démarrer le compte à rebours de la vidéo", key="start_sync_timer"):
...
if st.session_state.start_countdown and not st.session_state.ad_played:
    ...
    for percent_complete in range(100):
        ...
        status_text.warning(f"⏳ Synchronisation en cours... Veuillez patienter {secondes_restantes}s (Vérification du flux publicitaire vidéo)")
...
if st.session_state.ad_played:
    st.success("✅ Vérification terminée avec succès ! Le flux est débloqué.")
    # Le bouton vert pointe désormais vers le vrai direct fonctionnel d'Equidia
    st.markdown("""
        <a href="https://equidia.fr" target="_blank" style="text-decoration:none;">
            <button style="background-color:#22c55e; color:white; border:none; padding:14px 25px; font-size:16px; font-weight:bold; border-radius:6px; cursor:pointer; width:100%;">
                📺 ACCÉDER IMMÉDIATEMENT AU DIRECT OFFICIEL EQUIDIA
            </button>
        </a>
    """, unsafe_allow_html=True)
    if st.button("🔄 Relancer une nouvelle course / Réinitialiser", key="reset_ad_btn"):
        st.session_state.ad_played = False
        st.rerun()
