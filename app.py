...
def auto_charger_course_pmu(reunion="R1", course="C1"):
    ...
    for i in range(1, total_secours + 1):
        secours_cotes[i] = {"nom": banque_noms[(i - 1) % len(banque_noms)], "cote": round(float(5.2 + ((i - 1) * 2.2)), 1)}
...
# --- [STRATÉGIE IMAGE FIXE] : BANNIÈRE DU HAUT INCASSABLE ET IMPARABLE ---
# Remplace l'URL de l'image par ton propre visuel ou une image hébergée en ligne
st.markdown("""
<div style="text-align: center; margin-bottom: 20px;">
    <a href="https://www.highrevenueformat.com/b9b40b1c4e412de03b6465589ab82662/invoke.js" target="_blank">
        <img src="https://unsplash.com" alt="Espace Partenaire" style="border-radius: 8px; max-width: 100%; height: auto; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
    </a>
</div>
""", unsafe_allow_html=True)
...
with col2:
    ...
    if not st.session_state.ad_played:
        ...
        cotes_triees = sorted(donnees_chevaux.items(), key=lambda x: x["cote"])
        num_liste_type = [int(item) for item in cotes_triees]
        ...
        masse_enjeux_totale = sum([100000 / item["cote"] for item in donnees_chevaux.items()]) * (1.0 - t_simple)
        st.metric(label="Masse Estimée des Enjeux Nettoyée", value=f"{masse_enjeux_totale:,.2f} €")

# --- 7. FLUX DE DÉTAILS ---
if st.session_state.ad_played:
    st.markdown("---")
    st.markdown("#### 📋 Liste complète des chevaux et cotes")
...
# --- [PUBLICITÉS 2 & 3] : ZONE PUBLICITAIRE DE BASE AVEC SMART SCRIPT ---
st.markdown("---")
