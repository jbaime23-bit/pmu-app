...
# --- NOUVEL ALGORITHME DE TRI DYNAMIQUE PAR COTES ---
num_ia_trie = []
# Tri de la structure d'après la cote numérique réelle (la plus petite cote/le favori d'abord)
cotes_triees_par_valeur = sorted(cotes_s.items(), key=lambda x: x[1])

for cheval_nom, _ in cotes_triees_par_valeur:
    try:
        # Analyse sécurisée du nom textuel ("N°3 - Idao de Tillard")
        if "N°" in cheval_nom:
            partie_gauche = cheval_nom.split(" - ")[0]
            txt_brut_num = partie_gauche.replace("N°", "").strip()
            if txt_brut_num.isdigit():
                num_ia_trie.append(int(txt_brut_num))
    except Exception:
        pass

# Répartition selon votre logique de pronostics PMU
...
# --- LISTE DES PARTICIPANTS CORRIGÉE (100% SÉCURISÉE SANS CRASH) ---
...
for cheval_complet, mise_calc in st.session_state.mises.items():
    try:
        if " - " in cheval_complet:
            morceaux = cheval_complet.split(" - ")
            txt_num_unitaire = morceaux[0].replace("N°", "").strip()
            txt_nom_unitaire = morceaux[1].strip()
