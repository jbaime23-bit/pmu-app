...
# --- [MONÉTISATION 1] : NOUVELLE STRATÉGIE CONTOURNEMENT IFRAME PURE (Adsterra) ---
# Cette méthode encapsule la publicité dans un document HTML complet pour forcer l'affichage sur mobile
code_adsterra_iframe = """
<iframe src="about:blank" srcdoc="
    <html>
    <head>
        <meta name='viewport' content='width=device-width, initial-scale=1.0'>
        <style>body {margin:0; padding:0; text-align:center; display:flex; justify-content:center; align-items:center; min-height:90px;}</style>
    </head>
    <body>
...
"""
st.markdown(code_adsterra_iframe, unsafe_allow_html=True)
...
with col1:
    ...
    if not st.session_state.start_countdown and not st.session_state.ad_played:
        if st.button("⚡ Générer la Sélection Stratégique", type="primary"):
            ...
            # Script Interstitiel encapsulé de la même façon
            code_popunder_iframe = """
            <iframe src="about:blank" srcdoc="
            ...
            """
            st.markdown(code_popunder_iframe, unsafe_allow_html=True)
            st.rerun()
...
with col2:
    ...
    if not st.session_state.ad_played:
        ...
        # Tri basé sur la valeur numérique de la cote
