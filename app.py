...
def trouver_course_pmu_actuelle():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # Parcourir les réunions pour trouver la course la plus proche du moment actuel
            for r in reunions:
                num_r = str(r.get("numOfficiel", 1))
                for c in r.get("courses", []):
                    num_c = str(c.get("numOfficiel", 1))
                    heure_depart = c.get("heureDepart", 0)
                    # Si la course part dans le futur ou s'est déroulée il y a moins de 15 minutes
                    if heure_depart > (maintenant - 900000):
                        return num_r, num_c
    except:
        pass
    return "1", "1"  # Valeur de secours si l'API ne répond pas
...
# --- AUTOMATISATION PAR DEFAUT : DÉTECTION ET SÉLECTION AUTOMATIQUE ---
