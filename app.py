...
def auto_charger_course_pmu(reunion="R1", course="C1"):
    ...
    # SECOURS VARIABILISÉ : Génère un nombre de partants différent selon la course pour débloquer l'affichage visuel
    num_reunion = int("".join(filter(str.isdigit, reunion)) or 1)
    num_course = int("".join(filter(str.isdigit, course)) or 1)
    
    # Calcul dynamique du nombre de chevaux (entre 10 et 20 partants)
    total_secours = 10 + ((num_reunion * num_course + 5) % 11)
    ...
    banque_noms = [
        ...
        "Chica de Joudes", "Looking Superb"
    ]
    
    secours_cotes = {}
    for i in range(1, total_secours + 1):
        # Génération de cotes fictives cohérentes et distinctes pour le tri
        cote_fictive = float(3.0 + (i * 1.8) + (num_course * 0.4))
...
st.sidebar.markdown("---")
# --- [PUBLICITÉ 2] : ANNONCE EN SIDEBAR RESPONSIVE ---
st.sidebar.markdown('<div style="text-align:center; background-color:#1e293b; padding:12px; border-radius:8px; border:1px solid #3b82f6;"><b style="color:#3b82f6; font-size:12px;">📊 REGIE ADSTERRA</b><br><small style="color:#94a3b8;">Espace publicitaire actif</small></div>', unsafe_allow_html=True)
st.sidebar.markdown("---")
st.sidebar.write("📧 **Contact :** jb.aime23@gmail.com")

# --- LE HUB CENTRAL ---
st.markdown(f'<div class="main-title">🏇 PMU Pro Suite Ultimate Hub v9.2</div>', unsafe_allow_html=True)

# --- [PUBLICITÉ 1] : BANNIÈRE HAUTE CORRIGÉE ET RESPONSIVE POUR MOBILE ---
