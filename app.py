...
# --- TRI NUMÉRIQUE DYNAMIQUE DES CHEVAUX POUR L'IA ---
num_ia_trie = []
# Tri basé sur la valeur de la cote (du favori au tocard)
...
for cheval_nom, _ in cotes_triees_par_valeur:
    try:
        if " - " in cheval_nom:
            ...
            num_extraction = "".join(filter(str.isdigit, partie_gauche))
            if num_extraction.isdigit():
                num_ia_trie.append(int(num_extraction))
    except Exception:
        pass

# Répartition selon vos critères hippiques
...
# --- LISTE DES PARTICIPANTS DYNAMIQUE (FORMAT CORRIGÉ) ---
