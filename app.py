...
# --- CRÉATION DE LA LISTE TYPE ET APPLICATION DE VOTRE FORMULE ---
liste_couple_triee = []

for cle_nom, valeur_cote in cotes_s.items():
    try:
        # Extrait uniquement les chiffres du numéro (ex: "N°5" -> 5)
        chiffres_bruts = "".join([c for c in cle_nom if c.isdigit()])
        if chiffres_bruts.isdigit():
            liste_couple_triee.append((int(chiffres_bruts), float(valeur_cote)))
    except:
        pass

# ÉTAPE 1 : Classer de la plus petite cote à la plus grande cote
liste_couple_triee.sort(key=lambda x: x)

# ÉTAPE 2 : CORRECTION EXTRACTION - On n'extrait QUE les numéros dans l'ordre de la Liste Type
num_liste_type = [couple[0] for couple in list_couple_triee]

combinaison_ia_9 = []

# ÉTAPE 3 : Choisi entre les 4 premières cotes les plus basses -> 2 chiffres
if len(num_liste_type) >= 4:
    palier_1 = num_liste_type[0:4]
    combinaison_ia_9.extend(palier_1[0:2])

# ÉTAPE 4 : Choisir 1 chiffre entre les 2 cotes suivantes
if len(num_liste_type) >= 6:
    palier_2 = num_liste_type[4:6]
    combinaison_ia_9.extend(palier_2[0:1])

# ÉTAPE 5 : Choisir 2 numéros dans les 3 cotes suivantes
if len(num_liste_type) >= 9:
    palier_3 = num_liste_type[6:9]
    combinaison_ia_9.extend(palier_3[0:2])

# ÉTAPE 6 : Choisir 2 numéros dans les 3 cotes suivantes
if len(num_liste_type) >= 12:
    palier_4 = num_liste_type[9:12]
    combinaison_ia_9.extend(palier_4[0:2])

# ÉTAPE 7 : Le reste choisi 2 chiffres dans les cotes restantes, en éliminant les 2 derniers
if len(num_liste_type) > 14:
    palier_restant = num_liste_type[12:-2]
    combinaison_ia_9.extend(palier_restant[0:2])
elif len(num_liste_type) > 12:
    palier_restant = num_liste_type[12:]
    combinaison_ia_9.extend(palier_restant[0:2])

# Sécurité pour toujours garantir exactement 9 numéros uniques
