...
# --- RECONSTRUCTION COMPLÈTE DU TRI DE L'IA PAR COTE NUMÉRIQUE ---
num_ia_trie = []
...
for cheval_nom, _ in cotes_triees_par_valeur:
    try:
        if " - " in cheval_nom:
            txt_num = cheval_nom.split(" - ")[0].replace("N°", "").strip()
            if txt_num.isdigit():
                num_ia_trie.append(int(txt_num))
    except Exception:
        pass

# Algorithme de répartition de vos numéros
...
# --- LISTE DES PARTICIPANTS DYNAMIQUE (FIXÉE SANS SCRIPT ERROR) ---
...
for cheval_complet, mise_calc in st.session_state.mises.items():
    try:
        if " - " in cheval_complet:
            ...
            txt_num = parties[0].replace("N°", "").strip()
            txt_nom = parties[1].strip()
            cote_associee = cotes_s.get(cheval_complet, 99.0)
            
            donnees_tableau.append({
                "Numéro": int(txt_num) if txt_num.isdigit() else txt_num,
                "Nom du Cheval": txt_nom,
