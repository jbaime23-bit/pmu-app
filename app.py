...
# --- TRI NUMÉRIQUE ET REPARTITION ANALYTIQUE EN ARRIÈRE-PLAN ---
num_ia_trie = []
cotes_triees_par_valeur = sorted(cotes_s.items(), key=lambda x: x[1])

for cheval_nom, _ in cotes_triees_par_valeur:
    try:
        # Extraction robuste du numéro du cheval à partir de la clé d'origine
        partie_num = "".join([c for c in cheval_nom.split(" - ")[0] if c.isdigit()])
        if partie_num.isdigit():
            num_ia_trie.append(int(partie_num))
    except Exception:
        pass

# Application stricte de vos règles de répartition d'analyse PMU
