...
def auto_charger_course_pmu(reunion="R1", course="C1", type_course="Attelé"):
    ...
    banque_noms_attele = ["Ampia Mede Sm", "Flamme du Goutier", "San Moteur", "Don Fanucci Zet", "Vivid Wise As", "Delia du Pommeux", "Horsy Dream", "Go On Boy", "Galius", "Etonnant", "Idao de Tillard", "Face Time Bourbon", "Diable de Vauvert", "Hohneck", "Hooker Berry"]
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # Forçage intelligent si la discipline réelle n'est pas du Trot
            ...
            if partants_data:
                ...
                for idx, p in enumerate(partants_data, 1):
                    ...
                    if $.get("live_odds") is None:
                        cote_pmu = float(5.2 + (idx * 1.5))
...
# --- [PUBLICITÉ 1] : BLOC DE TÊTE SÉCURISÉ NATIVE HAUT ---
code_adsterra_haut = """
...
with col2:
    ...
    if not st.session_state.ad_played:
        ...
        # Tri et extraction correcte des valeurs numériques
...
liste_triee_affichage = sorted(donnees_chevaux.items(), key=lambda x: x)
for n_chv, info_chv in liste_triee_affichage:
    st.write(f"🏇 **Cheval N°{n_chv}** : {info_chv['nom']} — 📊 Cote réelle : `{info_chv['cote']}`")

# --- 8. ZONE PUBLICITAIRE DU BAS RESTAURÉE NATIVE ---
st.markdown("---")
