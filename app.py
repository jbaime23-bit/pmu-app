...
course_choisie = st.sidebar.selectbox("Course :", [f"C{x}" for x in range(1, 13)], index=0) # Index 0 pour C1 par défaut
t_simple = st.sidebar.slider("Taxe Simple (%)", 0, 30, 15) / 100

st.sidebar.markdown("---")
st.sidebar.write("📧 **Contact :** jb.aime23@gmail.com")

# Identifiant unique combiné pour casser le cache persistant
id_course_unique = f"{reunion_choisie}_{course_choisie}"

# --- 3. INITIALISATION ET NETTOYAGE AUTOMATIQUE DU CACHE ---
if 'current_race' not in st.session_state:
    st.session_state.current_race = id_course_unique

# Si l'utilisateur change de course, on réinitialise l'état pour effacer les blocages de la course 4
...
# --- 4. SYNCHRONISATION DYNAMIQUE DES PARTANTS ---
def obtenir_donnees_pmu_temps_reel(reunion, course, type_course):
    ...
    for i in range(1, total_partants + 1):
        # Utilisation des numéros de course pour faire varier mathématiquement les cotes
...
# --- 5. EN-TÊTE ET PUBLICITÉ DU HAUT FORCEE ---
st.title("🏆 PMU PRO")
...
"""
# La clé dynamique force le rechargement du composant publicitaire
components.html(code_adsterra_haut, height=100, scrolling=False, key=f"ad_top_{id_course_unique}")
...
# --- 9. ZONE PUBLICITAIRE DU BAS FORCEE ET NETTOYÉE ---
st.markdown("---")
...
"""
components.html(code_publicites_bas, height=120, scrolling=False, key=f"ad_bot_{id_course_unique}")
