...
# --- SYSTEME DE SYNCHRONISATION DIRECTE ---
def recuperer_donnees_pmu_reelles(reunion="1", course="1"):
    import datetime
    date_jour = datetime.date.today().strftime("%d%m%Y")
    url_p = f"https://pmu.fr{date_jour}/R{reunion}/C{course}/participants"
    url_r = f"https://pmu.fr{date_jour}/R{reunion}/C{course}/rapports-probables?typePari=SIMPLE_GAGNANT"
    try:
        ...
        if res_p.status_code == 200:
            ...
            if partants_data:
                mises_calculees = {}
                res_r = requests.get(url_r, timeout=4)
                cotes_dict = {}
...
# INTERFACE INTERACTIVE (MISE A JOUR DE VERSION FORCEE v7.4)
st.title(" 🏇 PMU Pro Suite Ultimate Hub v7.4")
st.info(f"👥 **Nombre total de visiteurs sur la plateforme : {st.session_state.total_visiteurs}**")

# REORGANISATION DES ONGLETS SANS AUCUNE STRUCTURE IMBRIQUÉE SENSIBLE
onglet1, onglet2, onglet3 = st.tabs(["📊 Calculateur", "🏆 Combinés", "🤖 IA & Règles"])
...
with onglet2:
    ...
    st.write(f"Tickets Couplés réalisables avec vos 9 chiffres actuels (**{st.session_state.combinaison_9_sauvegardee}**) :")
    ...
    if not st.session_state.video_vue and not st.session_state.est_admin:
        ...
        for num in liste_9_chiffres:
            ...
            if 1 <= num <= 4:
                ...
                note_regle, confiance = "Outsider Spéculatif (Règle Expert).", confiance + 25
            elif num >= 13:
                note_regle, confiance = "Grosse Cote / Tocard.", confiance - 10
        ...
        if st.button("🔄 Relancer une analyse IA"):
