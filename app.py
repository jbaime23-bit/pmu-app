...
# 📢 --- [PUBLICITÉ 1/3] : BANNIÈRE HAUT ADSTERRA ---
code_adsterra_haut = """
...
with col1:
    ...
    # 📢 --- [PUBLICITÉ 2/3] : NOUVEAU BLOC INTÉGRÉ CPM NETWORK (Anciennement bloc vidéo buggé) ---
    if st.session_state.start_countdown and not st.session_state.ad_played:
        st.markdown("#### 🎬 Espace Partenaire Sponsorisé")
        
        # Insertion propre et centrée de votre premier code CPM Network pour affichage normal
        code_cpm_milieu = """
        <div style="width: 100%; text-align: center; margin-bottom: 15px; padding: 10px; background-color: #f8fafc; border-radius: 8px;">
            <script src="https://pl31412908.profitableratecpmnetwork.com/20/e7/77/20e777896497a03cbf50c084a66999b4.js"></script>
        </div>
        """
        components.html(code_cpm_milieu, height=110, scrolling=False)
        
        progress_bar = st.progress(0)
        status_text = st.empty()
        
        # Gestion stricte du temps (10 secondes)
...
st.markdown("---")
st.markdown(f"#### 📋 Partants et Cotes Actuels ({total_partants} chevaux détectés)")
