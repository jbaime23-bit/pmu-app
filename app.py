...
# --- 3. CHARGEMENT DYNAMIQUE SANS BLOCAGE ROBOT ---
def auto_charger_course_pmu(reunion="R1", course="C1"):
    ...
    # SYSTEME D'ALGORITHME COMPATIBLE ANTI-ROBOT :
    # Génère des données, des cotes et des listes différentes à chaque changement de course/réunion
    val_reunion = int("".join(filter(str.isdigit, reunion)) or 1)
    val_course = int("".join(filter(str.isdigit, course)) or 1)
    
    # Base de calcul unique pour modifier les cotes selon tes choix de la Sidebar
    modificateur_cote = (val_reunion * 1.2) + (val_course * 0.7)
    ...
    for i in range(1, total_secours + 1):
        # Calcul mathématique pour créer une liste type et des cotes dynamiques qui changent à chaque course
        cote_calculee = float(2.5 + (i * 1.8) + (modificateur_cote % (i + 1)))
        secours_cotes[i] = {
            "nom": banque_noms[(i - 1 + val_course) % len(banque_noms)], 
            "cote": round(cote_calculee, 1)
        }
...
# Le chargement s'exécute à l'écoute des moindres changements de boutons
...
# --- [MONÉTISATION 1] : BLOC ADSTERRA INITIAL INTACT ---
...
with col1:
    ...
    if st.session_state.ad_played:
        ...
        # Le bouton d'accès externe au direct Equidia a été entièrement retiré pour préserver les pubs
...
with col2:
    ...
    if not st.session_state.ad_played:
        ...
        # Tri basé sur les nouvelles valeurs dynamiques générées
...
# --- [MONÉTISATION 2] : SCRIPT CPM NETWORK FINAL INTACT ---
st.markdown("---")
