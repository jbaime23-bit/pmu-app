...
# Tri et traitement sécurisé de la combinaison IA
...
# --- BANNIÈRE PUBLICITAIRE N°1 CONSERVÉE (728x90) ---
...
# --- STRUCTURE DU LECTEUR VIDEO DE 5 SECONDES REPAREE ---
...
if not st.session_state.start_countdown and not st.session_state.ad_played:
    # Ce lien ouvre proprement votre régie publicitaire et un bouton déclenche le décompte
    st.markdown("""
        <a href="https://profitableratecpmnetwork.com" target="_blank" style="text-decoration:none;">
            <button style="background-color:#ef4444; color:white; border:none; padding:14px 25px; font-size:16px; font-weight:bold; border-radius:6px; cursor:pointer; width:100%;">
                ▶️ CLIQUEZ ICI POUR CONFIGURER LE FLUX VIDEO PUBLICITAIRE
            </button>
        </a>
    """, unsafe_allow_html=True)
    
    if st.button("⚡ Lancer les 5 secondes de synchronisation vidéo", type="secondary"):
...
if st.session_state.start_countdown and not st.session_state.ad_played:
    ...
    for percent_complete in range(100):
        ...
        status_text.warning(f"⏳ Synchronisation en cours... Veuillez patienter {secondes_restantes}s (Vidéo active dans l'autre onglet)")
...
if st.session_state.ad_played:
    st.success("🎥 Flux vidéo débloqué ! Regardez le streaming en direct ci-dessous :")
    
    # LECTEUR EQUIDIA DIRECT PARFAITEMENT STRUCTURE AU CENTRE DE LA PAGE
    code_equidia_embed = """
    <div style="text-align:center; width:100%; border-radius:8px; overflow:hidden; border:2px solid #22c55e; margin-bottom:15px;">
        <iframe src="https://equidia.fr" width="100%" height="450" frameborder="0" scrolling="yes" allowfullscreen></iframe>
    </div>
    """
    components.html(code_equidia_embed, height=470, scrolling=True)
    
    if st.button("🔄 Changer de course / Fermer le streaming", key="reset_ad_btn"):
        st.session_state.ad_played = False
        st.rerun()
...
with onglet1:
    ...
    # BANNIÈRE NATIVE ADSTERRA CONSERVÉE ET SÉCURISÉE
    code_adsterra_native_iframe = """
    <div style="text-align:center; width:100%;">
        <iframe src="https://highrevenueformat.com" width="320" height="150" frameborder="0" scrolling="no"></iframe>
    </div>
    """
    components.html(code_adsterra_native_iframe, height=160, scrolling=False)
