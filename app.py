...
# Rechargement dynamique immédiat lors du changement de sélection
...
# --- TRI NUMÉRIQUE PAR COTE DES CHEVAUX POUR L'IA ---
num_ia_trie = []
# Tri basé sur les valeurs réelles des cotes calculées (€)
cotes_triees_par_valeur = sorted(cotes_s.items(), key=lambda item: item[1])

for cheval_nom, _ in cotes_triees_par_valeur:
    try:
        # Extraction du numéro directement depuis la chaîne "N°X - Nom"
        if " - " in cheval_nom:
            txt_brut_num = cheval_nom.split(" - ")[0].replace("N°", "").strip()
            if txt_brut_num.isdigit():
                num_ia_trie.append(int(txt_brut_num))
    except Exception:
        pass

# Algorithme de répartition hippique
...
# --- LISTE DES PARTICIPANTS COMPLÈTEMENT SÉCURISÉE EN TABLEAU ---
...
for cheval_complet, mise_calc in st.session_state.mises.items():
    try:
        if " - " in cheval_complet:
            # Séparation propre et sécurisée par variables textuelles uniques
            division_texte = cheval_complet.split(" - ")
            valeur_texte_numero = division_texte[0].replace("N°", "").strip()
            valeur_texte_nom = division_texte[1].strip()
