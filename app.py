...
# --- TRI NUMÉRIQUE DYNAMIQUE DES CHEVAUX POUR L'IA (FIXÉ D'APRÈS LA COTE) ---
num_ia_trie = []
# Tri de la structure d'après la valeur réelle de la cote (du plus petit au plus grand)
cotes_triees_par_valeur = sorted(cotes_s.items(), key=lambda item: item[1])

for cheval_nom, _ in cotes_triees_par_valeur:
    try:
        if " - " in cheval_nom:
            partie_gauche = cheval_nom.split(" - ")[0]
            txt_num = "".join(filter(str.isdigit, partie_gauche))
            if txt_num.isdigit():
                num_ia_trie.append(int(txt_num))
    except Exception:
        pass

# Répartition selon votre logique algorithmique PMU
...
# --- LISTE DES PARTICIPANTS PROPRE EN TABLEAU (ZÉRO ERREUR SYNTAXE) ---
...
for cheval_complet, mise_calc in st.session_state.mises.items():
    try:
        if " - " in cheval_complet:
            ...
            str_num = parties[0].replace("N°", "").strip()
            str_nom = parties[1].strip()
