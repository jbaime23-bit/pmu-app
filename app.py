...
# Rechargement dynamique automatique immédiat
...
# --- APPLICATION DE VOTRE FORMULE DE SÉLECTION D'IA ---
liste_couple_triee = []

for cle_nom, valeur_cote in cotes_s.items():
    try:
        # Extraction du numéro du cheval (ex: "N°14 - ..." -> 14)
        chiffres_trouves = "".join([c for c in cle_nom.split(" - ")[0] if c.isdigit()])
        if chiffres_trouves.isdigit():
            liste_couple_triee.append((int(chiffres_trouves), float(valeur_cote)))
    except:
        pass

# ÉTAPE 1 : Classer de la plus petite cote à la plus grande cote pour créer la Liste Type
liste_couple_triee.sort(key=lambda x: x[1])

# ÉTAPE 2 : Extraction stricte des numéros seuls dans l'ordre de la Liste Type
num_liste_type = [couple[0] for couple in liste_couple_triee]
