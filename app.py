...
# Rechargement dynamique automatique immédiat lors du changement
...
# --- NOUVELLE LOGIQUE DE TRI ET FORMULE D'ANALYSE DYNAMIQUE DE L'IA ---
liste_chevaux_ordonnee = []

for cle_nom, valeur_cote in cotes_s.items():
    try:
        # Extraction du numéro brut du cheval (ex: "N°4" -> 4)
        chiffres_trouves = "".join([c for c in cle_nom if c.isdigit()])
        if chiffres_trouves.isdigit():
            liste_chevaux_ordonnee.append((int(chiffres_trouves), float(valeur_cote)))
    except:
        pass

# ÉTAPE 1 : Classer de la plus petite cote à la plus grande cote
liste_chevaux_ordonnee.sort(key=lambda x: x[1])

# Extraction ordonnée uniquement des numéros des chevaux triés
num_ia_trie = [info[0] for info in liste_chevaux_ordonnee]

combinaison_ia_9 = []

# ÉTAPE 2 : Choisi entre les 4 premiers numéros (cotes les plus basses) -> 2 chiffres
palier_1 = num_ia_trie[0:4]
combinaison_ia_9.extend(palier_1[0:2])

# ÉTAPE 3 : Choisir 1 chiffre entre les 2 cotes suivantes
palier_2 = num_ia_trie[4:6]
combinaison_ia_9.extend(palier_2[0:1])

# ÉTAPE 4 : Choisie 2 numéros dans les 3 cotes suivantes
palier_3 = num_ia_trie[6:9]
combinaison_ia_9.extend(palier_3[0:2])

# ÉTAPE 5 : Encore choisis 2 numéros encore dans les 3 cotes suivantes
palier_4 = num_ia_trie[9:12]
combinaison_ia_9.extend(palier_4[0:2])

# ÉTAPE 6 : Le reste choisis 2 chiffres dans les cotes restantes, MAIS sans jamais choisir les deux derniers
if len(num_ia_trie) > 14:
    palier_restant = num_ia_trie[12:-2]  # Exclut strictement les 2 dernières cotes les plus élevées
else:
    palier_restant = num_ia_trie[12:]
combinaison_ia_9.extend(palier_restant[0:2])

# Sécurité mathématique : Si la course manque de partants, on remplit jusqu'à obtenir 9 chiffres uniques
for n in num_ia_trie:
