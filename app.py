...
# --- 3. CHARGEMENT DYNAMIQUE AVEC SIMULATION DE COTES RÉELLES ---
def auto_charger_course_pmu(reunion="R1", course="C1"):
    ...
    # Solution de secours réaliste anti-blocage
    ...
    return f"{reunion} {course} (Données Simulées Live)", total_secours, secours_cotes
...
with col2:
    ...
    if not st.session_state.ad_played:
        ...
        # Tri des chevaux par cotes pour bâtir la Liste Type ordonnée
        liste_triee = sorted(donnees_chevaux.items(), key=lambda x: x[1]["cote"])
        
        # Liste ordonnée uniquement selon la valeur croissante des cotes
        num_liste_type = [int(item[0]) for item in liste_triee]

        combinaison_ia_9 = []

        # Application stricte de vos paliers sur la Liste Type ordonnée
...
# --- 7. FLUX DE DÉTAILS TRIVAL PAR COTES (La vraie Liste Type visuelle) ---
if st.session_state.ad_played:
    st.markdown("---")
    st.markdown("#### 📋 Liste complète des chevaux et cotes (Triée par Liste Type — Favoris en premier)")
    
    # CORRECTION : On affiche la liste triée par la valeur de la COTE (Liste Type réelle)
