...
if 'selection_chiffres' not in st.session_state: st.session_state.selection_chiffres = "1-2-3-4-5-6-7-8-9"

# BARRE LATÉRALE PUBLICITÉS (VISUELLE)
st.sidebar.header("📢 Espace Sponsor Premium")

# Bloc d'attente coloré pour Adsterra
st.sidebar.markdown("""
<div style="text-align:center; background-color:#eff6ff; padding:15px; border-radius:8px; border:2px solid #3b82f6;">
    <b style="color:#1e40af; font-size:12px;">📊 ANNONCE SIDEBAR ACTIVE</b><br>
    <small style="color:#1d4ed8;">Votre code Adsterra s'affichera ici</small>
</div>
...
# INTERFACE INTERACTIVE v8.1
st.title(" 🏇 PMU Pro Suite Ultimate Hub v8.1")
st.info(f"👥 **Nombre total de visiteurs sur la plateforme : {st.session_state.total_visiteurs}**")

# Bloc d'attente haut de page coloré pour Adsterra
st.markdown("""
<div style="text-align:center; background-color:#fef08a; border:2px dashed #ca8a04; padding:12px; border-radius:8px; margin-bottom:15px;">
    <b style="color:#854d0e;">📢 ENCART PUBLICITAIRE BANNIÈRE HAUTE AUTOMATIQUE (ADSTERRA READY)</b>
</div>
""", unsafe_allow_html=True)

# --- 1. CONFIGURATION DU LIVE SÉCURISÉ SANS BLOCAGE ---
with st.expander("📺 Regarder la course Equidia / YouTube en Direct", expanded=True):
    # Utilisation de la fonction officielle Streamlit acceptée par YouTube
...
with onglet3:
    ...
    if not st.session_state.video_vue and not st.session_state.est_admin:
        ...
        for num in liste_9_chiffres:
            ...
            if 1 <= num <= 4:
                note_regle, confiance = "Favori Solide.", confiance + 20
            elif num == 5:
                note_regle, confiance = "Point d'appui incontournable.", confiance + 15
            elif 6 <= num <= 12:
