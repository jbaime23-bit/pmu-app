...
def auto_charger_course_pmu(reunion="R1", course="C1"):
    ...
    secours_cotes = {}
    total_secours = 13  # Fixé à 13 partants d'après tes observations visuelles de secours
    for i in range(1, total_secours + 1):
        secours_cotes[i] = {"nom": banque_noms[(i - 1) % len(banque_noms)], "cote": round(float(5.2 + ((i - 1) * 2.2) + (facteur_course * 0.2)), 1)}
...
# CORRECTION DE DYNAMISME : Le chargement s'exécute à chaque changement de la barre latérale
...
# --- [MONÉTISATION 1] : TES LIGNES ADSTERRA COMPLÈTEMENT INTACTES ---
...
with col1:
    ...
    if not st.session_state.start_countdown and not st.session_state.ad_played:
        if st.button("⚡ Générer la Sélection Stratégique", type="primary"):
            ...
            # --- [MONÉTISATION 2] : TA LIGNE CPM NETWORK COMPLÈTEMENT INTACTE ---
    ...
    if st.session_state.ad_played:
        ...
        # AJOUT DU BOUTON DIRECT LIVE SÉCURISÉ VISIBLE ICI
        st.markdown("""
            <div style="text-align: center; margin-top: 15px; margin-bottom: 10px;">
                <a href="https://equidia.fr" target="_blank" style="display: inline-block; padding: 12px 30px; background-color: #22c55e; color: white; font-weight: bold; text-decoration: none; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); font-size: 15px;">
...
with col2:
    ...
    if not st.session_state.ad_played:
        ...
        # Tri basé sur la valeur numérique de la cote recalculée dynamiquement
