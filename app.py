...
# CORRECTION : Rechargement dynamique immédiat sans bouton bloquant
...
# --- RECONSTRUCTION COMPLÈTE DU TRI DYNAMIQUE DE L'IA ---
num_ia_trie = []
# Tri basé sur la valeur réelle des cotes calculées (du plus petit au plus grand)
cotes_triees_par_valeur = sorted(cotes_s.items(), key=lambda item: item[1])

for cheval_nom, cote_val in cotes_triees_par_valeur:
    try:
        # Extraction propre du numéro de chaque cheval ("N°5" -> 5)
        texte_numero = cheval_nom.split(" - ")[0].replace("N°", "").strip()
        if texte_numero.isdigit():
            num_ia_trie.append(int(texte_numero))
    except Exception:
        pass

# Application de vos règles de répartition PMU sur la liste triée dynamiquement
...
# Sécurité pour toujours avoir exactement 9 chiffres uniques
...
# --- LISTE DES PARTICIPANTS CORRIGÉE (AFFICHAGE STRICT EN TABLEAU) ---
...
for cheval_complet, mise_calc in st.session_state.mises.items():
    try:
        ...
        parties = cheval_complet.split(" - ")
        num_cheval = parties[0].replace("N°", "").strip()
        nom_cheval = parties[1].strip() if len(parties) > 1 else "Inconnu"
