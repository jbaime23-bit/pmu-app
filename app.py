...
# Mémoire pour bloquer les cotes et empêcher le recalcul automatique
...
def forcer_rafraichissement():
    ...
    # On ne vide pas la mémoire ici pour garder les cotes figées, sauf changement de course
...
def charger_course_ultra_secondaire(reunion="R1", course="C1"):
    ...
    # 🛑 RÈGLE D'OR : Si la course a déjà été extraite une fois, on renvoie la mémoire sans rien recalculer
    ...
    # URL alternative (Exemple sur un agrégateur ou annuaire de turf léger sans Cloudflare)
    url_alternative = f"https://geny.com{reunion}-{course}-pronostics"
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # Simulation d'extraction des balises d'un tableau HTML classique de site secondaire
            lignes_tableau = soup.find_all('tr')
            for index, ligne in enumerate(lignes_tableau[1:], 1):
                num = index
                nom_cheval = f"Cheval_{num}"
                cote_pmu = float(4.0 + (index * 1.2))
            ...
            if mises_calculees:
                ...
                # Sauvegarde définitive dans la mémoire pour bloquer les changements
                st.session_state.donnees_courses_verrouillees[id_unique_course] = resultat
                return resultat
    except:
        pass
    
    # Générateur algorithmique fixe de secours (si hors-ligne) - Ne varie jamais après le premier calcul
...
# Appel de la fonction sécurisée à extraction unique
...
with col2:
    ...
    if not st.session_state.ad_played:
        ...
        # Tri et calculs basés sur les cotes verrouillées en mémoire
