...
def auto_charger_course_pmu(reunion="R1", course="C1"):
    ...
    # SECOURS VARIABILISÉ CORRIGÉ : Assure une variation dynamique jusqu'à 20 chevaux
    num_reunion = int("".join(filter(str.isdigit, reunion)) or 1)
    num_course = int("".join(filter(str.isdigit, course)) or 1)
    
    # Génération d'un nombre de partants variable (ex: 14 à 20 partants)
    total_secours = 14 + ((num_reunion + num_course) % 7)
    ...
    for i in range(1, total_secours + 1):
        cote_fictive = float(3.0 + (i * 1.5) + (num_course * 0.5))
...
# --- LE HUB CENTRAL ---
st.markdown(f'<div class="main-title">🏇 PMU Pro Suite Ultimate Hub v9.3</div>', unsafe_allow_html=True)
...
# --- CORRECTION STRICTE DU TRI ET DE LA SÉLECTION IA ---
