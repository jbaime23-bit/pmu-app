...
# Synchronisation et mise à jour automatique lors du changement de sélection
...
# --- REPARATION DU TRI ET DE LA SÉLECTION ALGORITHMIQUE IA (Changement dynamique) ---
cotes_triees = sorted(cotes_s.items(), key=lambda item: item[1])
num_ia_trie = []
for item in cotes_triees:
    # item[0] contient la chaîne "N°X - Nom du cheval"
    partie_numero = item[0].split(" - ")[0]
    num_extraction = "".join(filter(str.isdigit, partie_numero))
...
# --- LISTE DES PARTICIPANTS (CONSERVATION DES NOMS ET NUMÉROS DES CHEVAUX) ---
...
for cheval_complet, mise_calc in st.session_state.mises.items():
    # Découpage propre du texte "N°X - Nom du cheval"
    parties = cheval_complet.split(" - ")
    num_cheval = parties[0]
    nom_cheval = parties[1] if len(parties) > 1 else "Inconnu"
    cote_associee = cotes_s.get(cheval_complet, 99.0)
    
    donnees_tableau.append({
        "Numéro": num_cheval,
        "Nom du Cheval": nom_cheval,
