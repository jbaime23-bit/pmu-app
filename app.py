...
# --- ALGORITHME DE TRI DE LA SÉLECTION NUMÉRIQUE ---
liste_triee = sorted(donnees_chevaux.items(), key=lambda x: x[1]["cote"])
num_liste_type = [int(item[0]) for item in liste_triee]

combinaison_ia_9 = []

# Application stricte de vos paliers de sélection numérique sur la Liste Type
...
if st.session_state.ad_played:
    ...
    # Correction de la clé pour éviter le crash TypeError
    premier_cheval = combinaison_ia_9[0]
    cote_base_calcul = donnees_chevaux.get(premier_cheval, {"cote": 10.0})["cote"]
    gain_brut_estime = float(50000.0 * (cote_base_calcul / 2.5))
    gain_net_estime = gain_brut_estime * (1.0 - t_simple)
    
    st.markdown('<div class="pru-box">', unsafe_allow_html=True)
    st.markdown(f'<p style="color: #f8fafc; font-size: 16px;">📈 Estimation des Gains Nets (Taxe Appliquée) :</p>', unsafe_allow_html=True)
    st.markdown(f'<p style="color: #e11d48; font-size: 32px; font-weight: bold; margin: 5px 0;">{gain_net_estime:,.2f} €</p>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

    # RECONSTITUTION DE LA COHABITATION EN PARALLÈLE (DOUBLE ÉCRAN DE BAS)
    col_gauche, col_droite = st.columns(2)
    
    with col_gauche:
        st.markdown('<p style="color:white; font-weight:bold; margin-top:15px;">📋 GRILLE DES CHEVAUX</p>', unsafe_allow_html=True)
        tableau_partants = []
        for num, details in list_triee:
