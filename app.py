...
def extraire_un_numero(texte):
    ...
    if chiffres:
        return int(chiffres[0])  # CORRECTION : prend bien le premier élément de la liste
    return 1
...
def generer_9_chiffres_depuis_remarques(mises_dict, specialite):
    ...
    if zone_1:
        try:
            nb_a_prendre = random.choice([1, 2]) # CORRECTION : Choix entre 1 ou 2 numéros
    ...
    if zone_3:
        try:
            nb_a_prendre_z3 = random.choice([1, 2]) # CORRECTION : Choix entre 1 ou 2 numéros
