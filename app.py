...
# --- STRATÉGIE DE REPARTITION STATIQUE ET SÉCURISÉE DE L'IA ---
# Algorithme linéaire ultra-stable basé sur le nombre total de partants chargés
nombre_total_partants = len(cotes_s)
num_ia_trie = list(range(1, nombre_total_partants + 1))

# Répartition selon les critères PMU standards
...
# Ajustement pour s'assurer d'avoir toujours exactement 9 numéros
...
with col1:
    st.info(f"🔮 Sélection automatique IA (9 numéros) : **{combinaison_formatee_ia}**")
with col2:
    st.metric(label="💰 Masse Globale des Enjeux", value=f"{m_totale:,.2f} €")


# --- LISTE DES PARTICIPANTS ULTRA-STABLE SANS DÉCOUPAGE DE TEXTE ---
st.subheader("🏇 Liste officielle des Chevaux Engagés et Cotes Live")

donnees_tableau = []
for index_cle, (nom_cheval_complet, mise_calc) in enumerate(st.session_state.mises.items(), 1):
    cote_associee = cotes_s.get(nom_cheval_complet, 99.0)
    donnees_tableau.append({
        "Identifiant": f"Partant N°{index_cle}",
        "Désignation du Cheval": nom_cheval_complet,
        "Cote Live (€)": cote_associee,
        "Mise Estimée (€)": round(mise_calc, 2)
    })

if donnees_tableau:
    df_chevaux = pd.DataFrame(donnees_tableau)
    st.dataframe(df_chevaux, use_container_width=True, hide_index=True)
else:
    st.warning("Aucun cheval chargé pour cette course.")


# --- [MONÉTISATION 3] : DEUXIÈME BANNIÈRE PUBLICITAIRE EN BAS DE PAGE ---
st.markdown("---")
