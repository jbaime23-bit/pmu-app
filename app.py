...
def recuperer_course_automatique(reunion="1", course="1"):
    """
    Se connecte proprement à l'API Turfinfo du PMU pour extraire 
    le vrai nom de la course et le nombre exact de partants (jusqu'à 20+).
    """
    import datetime
    ...
    # Secours par défaut si le réseau échoue ou s'il n'y a pas de course en direct
...
st.sidebar.subheader("📡 Sélection Automatique Live")
...
st.title("🏇 PMU Pro Suite Ultimate Hub v4.7")
st.info(f"👥 **Nombre total de visiteurs sur la plateforme : {total_visiteurs}**")

onglet1, onglet2, onglet3, onglet4 = st.tabs(["📊 Calculateur Direct", "🧮 Stratégies d'Arbitrage", "🏆 Combinés Couplés", "🤖 Prédictions IA & Vos Règles"])

cotes_s, m_totale = calculer_jeu_simple(st.session_state.mises, t_simple)

with onglet1:
    st.header(f"📈 Analyse Active : {nom_course} ({type_course}) — {st.session_state.nombre_partants} Concurrents")
    
    st.subheader("🔢 Saisie de votre sélection de base (9 chiffres max)")
    chiffres_saisis = st.text_input("Entrez vos numéros séparés par des virgules :", max_chars=35, key="chiffres_clean_9")
    if chiffres_saisis:
