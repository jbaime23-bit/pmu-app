...
# Votre Direct Link Adsterra pour maximiser vos ouvertures de publicités mobiles
lien_direct_adsterra = "https://highrevenueformat.com"
...
if not st.session_state.start_countdown and not st.session_state.ad_played:
    ...
    <a href="{lien_direct_adsterra}" target="_blank" style="text-decoration:none;">
        <button style="background-color:#ef4444; color:white; border:none; padding:14px 25px; font-size:16px; font-weight:bold; border-radius:6px; cursor:pointer; width:100%;">
            ▶️ CLIQUEZ ICI POUR ACTIVER LA VIDÉO ET CONFIGURER LE LIVE
        </button>
    </a>
    """
    st.markdown(html_bouton_ad, unsafe_allow_html=True)
    
    if st.button("⚡ Cliquer ici juste après pour démarrer le compte à rebours de 5s", key="start_sync_timer"):
...
if st.session_state.start_countdown and not st.session_state.ad_played:
    ...
    for percent_complete in range(100):
        ...
        status_text.warning(f"⏳ Synchronisation en cours... Veuillez patienter {secondes_restantes}s (Publicité vidéo ouverte dans l'autre onglet)")
...
if st.session_state.ad_played:
    st.success("✅ Vérification terminée ! Le flux est débloqué.")
    st.markdown("""
    ...
    """, unsafe_allow_html=True)
    if st.button("🔄 Relancer une nouvelle session (Réinitialiser)", key="reset_ad_btn"):
        st.session_state.ad_played = False
        st.rerun()
