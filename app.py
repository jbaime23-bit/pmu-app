...
def extraire_un_numero(texte):
    ...
    if chiffres:
        return int(chiffres[0]) # CORRECTION : Extraction propre du premier chiffre
    return 1
...
def generer_9_chiffres_depuis_remarques(mises_dict, specialite):
    ...
    if zone_1:
        try:
            nb_a_prendre = random.choice([1, 2]) # CORRECTION : Rétablissement des choix
    ...
    if zone_3:
        try:
            nb_a_prendre_z3 = random.choice([1, 2]) # CORRECTION : Rétablissement des choix
