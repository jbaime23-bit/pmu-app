...
with onglet1:
    ...
    if chiffres_saisis:
        ...
        if len(liste_9) > 9:
            st.error(f"⚠️ Limite de 9 chiffres dépassée ! ({len(liste_9)} entrés)")
        else:
            st.success(f"✅ Vos {len(liste_9)} numéros enregistrés : {', '.join(liste_9)}")

    st.divider()
    st.subheader("Ajuster les Enjeux Financiers du Jour (€)")
    cles_chevaux = list(st.session_state.mises.keys())
    milieu = len(cles_chevaux) // 2
