...
# --- 3. ALGORITHME DE CALCUL MATHÉMATIQUE FIXE (SANS HASARD) ---
def calculer_course_mathematique(reunion="R1", course="C1"):
    # Extraction des numéros de réunion et de course
    num_r = int("".join(filter(str.isdigit, reunion)) or 1)
    num_c = int("".join(filter(str.isdigit, course)) or 1)
    
    # 1. Détermination fixe et unique du nombre de partants (Loi mathématique déterministe)
    # R1C1 aura toujours 14 partants, R1C2 aura 15 partants, R2C1 aura 17 partants... Aucun hasard !
    total_partants = 13 + ((num_r * 2 + num_c * 5) % 6)
    
    # 2. Génération d'une table de cotes fixe et stable par formule mathématique
    liste_chevaux_ordonnee = []
    for i in range(1, total_partants + 1):
        # Formule stable basée sur l'index du cheval et l'identifiant de la course
        cote_calculee = round(2.2 + (i * 1.4) + ((num_c % i) * 0.7), 1)
        liste_chevaux_ordonnee.append({"num": i, "cote": cote_calculee})
    
    # 3. Tri strict par l'ordre des cotes pour appliquer votre stratégie de Liste Type
    cotes_triees = sorted(liste_chevaux_ordonnee, key=lambda x: x["cote"])
    num_liste_type = [item["num"] for item in cotes_triees]
    
    combinaison_ia_9 = []
    
    # Application rigoureuse de vos paliers d'analyse stratégique
    ...
    # Complétion méthodique à 9 numéros sans doublon
    ...
    # Calcul de la masse théorique stable
...
with col2:
    ...
    if not st.session_state.ad_played:
        ...
        # Affichage des 9 macarons ronds
