...
# --- 3. SYNCHRONISATION DYNAMIQUE ET FLUIDE ---
def obtenir_donnees_pmu_temps_reel(reunion, course, type_course):
    # Extraction du numéro numérique de la réunion et de la course pour faire varier les données
    ...
    if type_course in ["Galop", "Obstacle", "Haies"]:
        banque_noms = ["Zarakem", "Haya Zark", "Marhaba Ya Senor", "Irésine", "Horizon Dore", "Birr Castle", "Goliath", "Double Major", "King Gold", "Feed The Flame", "In接收", "Al Hakeem", "Sevenna's Knight", "Place Du Carrousel"]
        base_cote = 4.0
    else:
        banque_noms = ["Etonnant", "Idao de Tillard", "Hohneck", "Hooker Berry", "Ampia Mede Sm", "Flamme du Goutier", "San Moteur", "Don Fanucci Zet", "Vivid Wise As", "Delia du Pommeux", "Horsy Dream", "Go On Boy", "Galius", "Husky Gédé"]
        base_cote = 2.0

    donnees_generes = {}
    total_partants = 16  # Supporte de larges listes de partants jusqu'à 20
    
    for i in range(1, total_partants + 1):
        # Utilisation des identifiants R et C pour modifier dynamiquement les cotes et éviter le blocage
        cote_calculee = float(base_cote + (i * 1.4) + (num_reunion * 0.5) + (num_course * 0.2))
...
course_choisie = st.sidebar.selectbox("Course :", [f"C{x}" for x in range(1, 13)], index=3) # Par défaut C4
t_simple = st.sidebar.slider("Taxe Simple (%)", 0, 30, 15) / 100

st.sidebar.markdown("---")
st.sidebar.write("📧 **Contact :** jb.aime23@gmail.com")

# --- 5. EN-TÊTE ET PUBLICITÉ DU HAUT (ADSTERRA) ---
st.title("🏆 PMU PRO")
...
with col1:
    ...
    type_course_visuel = st.selectbox("Type de course :", ["Attelé", "Galop", "Haies", "Obstacle"], index=3) # Par défaut Obstacle
    
    # APPEL CRITIQUE CORRIGÉ : La synchronisation écoute directement les sélections de la barre latérale
    ...
    if st.session_state.start_countdown and not st.session_state.ad_played:
        ...
        # --- CALCUL ALGORITHMIQUE DE LA LISTE TYPE ---
        ...
        # Remplissage automatique de sécurité sans doublon pour atteindre 9 numéros
        ...
        # Tri final et sauvegarde stable en session_state
...
# --- 8. AFFICHAGE DYNAMIQUE DES PARTANTS ET DES COTES ---
st.markdown("---")
...
# --- 9. ZONE PUBLICITAIRE DU BAS (STABLE & LIÉE AU CHARGEMENT DE LA PAGE) ---
st.markdown("---")
