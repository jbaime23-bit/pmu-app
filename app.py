...
# --- CONNECTEUR AMÉLIORÉ ET UNIVERSEL TOUTES COURSES ---
def recuperer_donnees_pmu_reelles(reunion="1", course="1"):
    import datetime
    date_jour = datetime.date.today().strftime("%d%m%Y")
    
    # Étape 1 : Lien universel principal pour obtenir la liste exacte de la course demandée
    ...
    try:
        ...
        if res_p.status_code == 200:
            ...
            if partants_data:
                mises_calculees = {}
                # On essaie d'avoir les vraies cotes
                ...
                # Construction de la liste dynamique de la course demandée
    ...
    # Secours automatique si le serveur PMU refuse la connexion complète
...
st.title(" 🏇 PMU Pro Suite Ultimate Hub v7.1")
...
with onglet2:
    ...
    diviseur = cote_couv - 1.0
    if diviseur <= 0:
        diviseur = 0.1
    mise_couverture_requise = round(mise_base / diviseur, 2)
    st.info(f"💡 **Règle d'arbitrage :** Pour couvrir entièrement votre risque, vous devez parier **{0.0 if ch_pref == ch_couv else mise_couverture_requise} €** sur le {ch_couv}.")
