...
<style>
    ...
    .number-badge {display: inline-block; background: linear-gradient(135deg, #e11d48, #be123c); color: white; font-size: 18px; font-weight: bold; padding: 8px 14px; margin: 4px; border-radius: 50%; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.2);}
...
st.sidebar.markdown("---")
# Remplacement de l'espace Sidebar par un bouton publicitaire Direct Link générant d'importants revenus au clic
st.sidebar.markdown('<p style="color:white; font-weight:bold; text-align:center;">🎁 BONUS GRATUIT</p>', unsafe_allow_html=True)
st.sidebar.markdown("""
    <div style="text-align: center;">
        <a href="https://highrevenueformat.com" target="_blank" style="display: inline-block; width: 100%; padding: 10px; background-color: #3b82f6; color: white; font-weight: bold; text-decoration: none; border-radius: 6px; font-size:12px;">
            💰 COLLECTER LE BONUS TURF ACTUEL
        </a>
    </div>
""", unsafe_allow_html=True)
st.sidebar.markdown("---")
...
# --- LE HUB CENTRAL ---
st.markdown(f'<div class="main-title">🏇 PMU Pro Suite Ultimate Hub v18.0</div>', unsafe_allow_html=True)

# --- ESPACE PUBLICITAIRE CENTRAL EN LIEN DIRECT FLEXIBLE (100% REVENUS DE CAPTATION) ---
st.markdown("""
    <div style="text-align: center; margin-bottom: 20px;">
        <a href="https://highrevenueformat.com" target="_blank" style="display: inline-block; padding: 12px 25px; background: linear-gradient(135deg, #3b82f6, #1d4ed8); color: white; font-weight: bold; text-decoration: none; border-radius: 8px; font-size: 14px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            🚀 PARIER AVEC LES MEILLEURES COTES DU JOUR (OFFRE ADSTERRA)
        </a>
    </div>
""", unsafe_allow_html=True)
...
if not st.session_state.start_countdown and not st.session_state.ad_played:
    st.markdown('<p class="info-text">Cliquez ci-dessous pour débloquer l\'accès au direct TV Youtube et lancer l\'analyse de l\'IA</p>', unsafe_allow_html=True)
    if st.button("▶️ DÉBLOQUER LE DIRECT TV & L'ANALYSE IA", key="trigger_btn", use_container_width=True, type="primary"):
...
if st.session_state.ad_played:
    ...
    st.markdown("""
        <div style="text-align: center; margin-top: 12px; margin-bottom: 10px;">
            <a href="https://youtube.com" target="_blank" style="display: inline-block; padding: 12px 30px; background-color: #ef4444; color: white; font-weight: bold; text-decoration: none; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                📺 ACCÉDER AU DIRECT LIVE TV (YOUTUBE HIPPIC)
            </a>
        </div>
    """, unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)

# --- ALGORITHME DE SÉLECTION DE L'IA ---
...
if st.session_state.ad_played:
    ...
    st.markdown('<p style="color:white; font-weight:bold; margin-top:15px;">📋 GRILLE DES CHEVAUX</p>', unsafe_allow_html=True)
    nums_list = []
    noms_list = []
    cotes_list = []
    for num, details in liste_triee:
        nums_list.append(int(num))
        noms_list.append(str(details["nom"]))
