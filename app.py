...
# INTERFACE INTERACTIVE v8.7
st.title(" 🏇 PMU Pro Suite Ultimate Hub v8.7")
st.info(f"👥 **Nombre total de visiteurs sur la plateforme : {st.session_state.total_visiteurs}**")

# --- SCRIPT ADSTERRA 1 (GRANDE BANNIÈRE DU HAUT 728x90) ---
...
# --- CONFIGURATION DU LIVE SÉCURISÉ PAR BOUTON UNIVERSEL ---
st.markdown("""
<div style="background-color:#ffffff; padding:20px; border-radius:10px; border:1px solid #e2e8f0; text-align:center; margin-bottom:20px;">
    <p style="margin:0 0 10px 0; font-weight:bold; color:#334155;">📺 Suivre le streaming des courses hippiques en direct</p>
    <a href="https://lescourseshippiques.fr" target="_blank" style="text-decoration:none;">
        <button style="background-color:#ef4444; color:white; border:none; padding:12px 25px; font-size:15px; font-weight:bold; border-radius:6px; cursor:pointer; width:100%; max-width:350px; box-shadow:0 4px 6px -1px rgb(0 0 0 / 0.1);">
            ▶️ LANCER LE LIVE VIDÉO EQUIDIA
        </button>
    </a>
</div>
...
with onglet3:
    ...
    if not st.session_state.video_vue and not st.session_state.est_admin:
        ...
        # --- SCRIPT ADSTERRA 2 (BANNIÈRE NATIVE LIÉE À L'ONGLET IA) ---
