...
with col2:
    ...
    if not st.session_state.ad_played:
        ...
        # Tri des chevaux par cotes pour obtenir l'ordre de la Liste Type
        liste_triee = sorted(donnees_chevaux.items(), key=lambda x: x[1]["cote"])
        
        # SÉLECTION CORRIGÉE : On extrait les positions ordonnées de la Liste Type (de 1 à X partants)
        # num_liste_type contient maintenant l'ordre sélectif en fonction de la valeur de la cote
        num_liste_type = [int(item[0]) for item in liste_triee]

        combinaison_ia_9 = []

        # Application stricte de vos paliers sur l'ordre de la Liste Type
...
if st.session_state.ad_played:
    ...
    for n_chv, info_chv in liste_triee_affichage:
        st.write(f" Romano 🏇 **Cheval N°{n_chv}** : {info_chv['nom']} — 📊 Cote réelle : `{info_chv['cote']}`")
