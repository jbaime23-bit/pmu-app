...
# --- RECONSTRUCTION DU TRI NUMÉRIQUE DE L'IA DÉPENDANT DES COTES ---
num_ia_trie = []
# Tri basé sur les valeurs numériques réelles des cotes calculées (le favori en premier)
cotes_triees_par_valeur = sorted(cotes_s.items(), key=lambda item: item[1])

for cheval_nom, _ in cotes_triees_par_valeur:
    try:
        # Extraction robuste de tous les chiffres présents dans la clé ("N°3 - Cheval" -> 3)
        chiffres_extraits = "".join([c for c in cheval_nom if c.isdigit()])
        if chiffres_extraits.isdigit():
            num_ia_trie.append(int(chiffres_extraits))
    except Exception:
        pass

# Répartition selon vos règles algorithmiques PMU
...
# --- LISTE DES PARTICIPANTS AU FORMAT TABLEAU SIMPLE ET ROBUSTE ---
st.subheader("🏇 Liste officielle des Chevaux Engagés et Cotes Live")

donnees_tableau = []
for index, (nom_complet_cle, mise_calculée) in enumerate(st.session_state.mises.items(), 1):
    try:
        cote_directe = cotes_s.get(nom_complet_cle, 99.0)
        donnees_tableau.append({
            "Position": index,
            "Désignation Complète": nom_complet_cle,
            "Cote Calculée (€)": cote_directe,
