...
def generer_9_chiffres_depuis_remarques(mises_dict, specialite):
    ...
    for k in mises_dict.keys():
        try:
            # Extraction 100% sécurisée du numéro de cheval (ex: "N°1 - Etonnant")
            if "N°" in k:
                texte_numero = k.split("-")[0]  # Récupère "N°1 "
                num_clean = texte_numero.replace("N°", "").strip() # Donne "1"
                numeros_disponibles.append(int(num_clean))
        except:
            continue
...
# --- LE SCRIPT ADSTERRA EMBARQUÉ DANS L'ENCART JAUNE VISUEL RESPONSIVE ---
