...
# --- ALGORITHME DE TRI DYNAMIQUE PAR COTE SÉCURISÉ ---
num_ia_trie = []
# CORRECTION DU TRI : On classe par la valeur de la cote (item[1]) et non pas par le texte
cotes_triees = sorted(cotes_s.items(), key=lambda item: item[1])

for cheval_nom, cote_val in cotes_triees:
    try:
        num_str = cheval_nom.split(" - ")[0].replace("N°", "").strip()
        if num_str.isdigit():
            num_ia_trie.append(int(num_str))
    except Exception:
        pass

# Répartition automatique de votre stratégie d'IA PMU
...
# --- LISTE DES PARTICIPANTS AU FORMAT TABLEAU (SIMPLE ET ULTRA-STABLE) ---
...
for cheval_complet, mise_calc in st.session_state.mises.items():
    try:
        part_num = cheval_complet.split(" - ")[0].replace("N°", "").strip()
        part_nom = cheval_complet.split(" - ")[1].strip()
        cote_associee = cotes_s.get(cheval_complet, 99.0)
        
        donnees_tableau.append({
            "Numéro": int(part_num) if part_num.isdigit() else part_num,
            "Nom du Cheval": part_nom,
