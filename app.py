...
# --- CHARGEMENT AUTOMATIQUE ET CORRIGÉ DU PROGRAMME PMU ---
def auto_charger_course_pmu(reunion="R1", course="C1"):
    # Formatage de la date du jour au format requis par l'API PMU : DDMMYYYY
    date_jour = datetime.date.today().strftime("%d%m%Y")
    timestamp_anti_blocage = int(time.time())
    
    # CORRECTION DE L'URL : Utilisation de l'endpoint officiel de l'API web du PMU
    ...
    try:
        ...
        if res_p.status_code == 200:
            ...
            # Si la structure contient une liste directe ou imbriquée
            ...
            if partants_list:
                ...
                for i, p in enumerate(partants_list, 1):
                    # Récupération du vrai numéro de programme, sinon index de boucle
                    num = p.get("numProg", p.get("numero", i))
                    nom_cheval = p.get("nom", "Inconnu")
                    
                    # Extraction de la cote directe ou de la dernière cote de référence disponible
                    cote_pmu = 15.0
    ...
    # Secours dynamique si l'API PMU ne répond pas ou est bloquée temporairement
...
# AJOUT D'UN BOUTON DE RAFRAÎCHISSEMENT DANS LA BARRE LATÉRALE POUR FORCER LA RECONNEXION
if st.sidebar.button("🔄 Rafraîchir les données Live", use_container_width=True):
    st.cache_data.clear()
    st.rerun()

# Récupération immédiate des données réelles PMU
...
if st.session_state.ad_played:
    ...
    # Section Affichage de la combinaison des 9 numéros
    st.markdown('<div class="pru-box">', unsafe_allow_html=True)
